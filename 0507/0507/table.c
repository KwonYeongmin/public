...
void outgame_content(int index, char *third, int second_line)
{
	...
	gotoxy(54, second_line);	setColor(15); second_box(index);
...
void second_box(int index)
{
	...
	{
		printf("몬스터를 피하여 아이템을 모아 목표지점까지 탈출하세요 ! \n \t\t\t\t\t키보드 방향키를 이용하여 움직일 수 있습니다.");
	}
	else if (index == 4)
	{
		printf("score : %d \n", g_info.score);
...
void ingame_table()
{
	...
	//테이블 크기 90X90, 맵 크기 88X88, 2칸씩 움직임
	setColor(1);
	...
	//gotoxy(110, 42);   update_data(g_info.score);
}
